...
public class ConventionsPlugin implements Plugin<Project> {
	...
	private void configureSpringJavaFormat(Project project) {
		...
		CheckstyleExtension checkstyleExt = project.getExtensions().getByType(CheckstyleExtension.class);
		checkstyleExt.setToolVersion(checkstyleToolVersion);
		checkstyleExt.getConfigDirectory().set(project.getRootProject().file("checkstyle"));
		...
		project.getTasks().withType(Format.class, (format) -> {
			...
			project.getTasks().withType(Checkstyle.class, (checkstyle) -> checkstyle.mustRunAfter(format));
			project.getTasks().withType(CheckFormat.class, (checkFormat) -> checkFormat.mustRunAfter(format));
		});
	}
	...
	private void configureSlf4j(Project project) {
		String slf4jVersion = (String) project.findProperty("slf4jVersion");

		project.getPlugins()
			.withType(JavaPlugin.class, (plugin) -> project.getDependencies()
				.add(JavaPlugin.IMPLEMENTATION_CONFIGURATION_NAME, "org.slf4j:slf4j-api:" + slf4jVersion));
	}
